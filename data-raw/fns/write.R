...
write_conditional_tags <- function(pkgs_chr,
  ...
  if(identical(where_1L_chr, character(0))){
    purrr::walk(c("R","DESCRIPTION"), ~ write_conditional_tags(pkgs_chr, path_to_pkg_root_1L_chr = path_to_pkg_root_1L_chr, consent_1L_chr = consent_1L_chr,
                                                               consent_indcs_int = consent_indcs_int, options_chr = options_chr, where_1L_chr = .x))
  }else{
    consented_fn <- function(path_to_pkg_root_1L_chr,#path_to_R_dir_1L_chr,
                             pkgs_chr,
                             where_1L_chr){
      if(where_1L_chr == "R"){
        path_to_R_dir_1L_chr <- paste0(path_to_pkg_root_1L_chr,"/R")
        paths_chr <- list.files(path_to_R_dir_1L_chr, full.names = TRUE)
        paths_chr[paths_chr %>%
                    purrr::map_lgl(~startsWith(.x,paste0(path_to_R_dir_1L_chr,"/","fn_"))|
                                     startsWith(.x,paste0(path_to_R_dir_1L_chr,"/","mthd_")))]  %>%
          purrr::walk(~{
            file_chr <- readLines(.x)
            file_chr <- purrr::reduce(pkgs_chr, .init = file_chr,
                                      ~{
                                        text_chr <- .x
                                        old_chr <- text_chr[text_chr %>% startsWith(paste0("#' @importFrom ", .y))]
                                        pkg_1L_chr <- .y
                                        new_chr <- old_chr %>% stringr::str_remove_all(paste0("#' @importFrom ", .y)) %>% trimws() %>% strsplit(split = " ") %>%
                                          purrr::map_chr(~paste0("#' @seealso ",
                                                                 paste0("[",pkg_1L_chr,"::",.x,"()]") %>%
                                                                   make_list_phrase()))
                                        text_chr[text_chr %in% old_chr] <- new_chr[match(text_chr, old_chr, nomatch = 0)]
                                        text_chr
        ...
        file_chr <- readLines(paste0(path_to_pkg_root_1L_chr,"/DESCRIPTION"))
        file_chr <- purrr::reduce(pkgs_chr, .init = file_chr,
                                  ~{
                                    text_chr <- .x
                                    text_chr[text_chr %>% purrr::map_chr(~trimws(.x)) %>% startsWith(.y)] <- NA_character_
                                    text_chr
                                  }) %>% purrr::discard(is.na)
        if(endsWith(file_chr[length(file_chr)],",")){
          file_chr[length(file_chr)] <- stringr::str_sub(file_chr[length(file_chr)],end=-2)
        }
        file_chr %>%
          writeLines(con = paste0(path_to_pkg_root_1L_chr,"/DESCRIPTION"))
    ...
    if(where_1L_chr == "R"){
      ...
      prompt_1L_chr <- paste0("Do you confirm that you want to overwrite files in directory '",
                              ...
                              make_list_phrase(pkgs_chr),
                              " and to instead use corresponding `@seealso` tags",
                              " ?")
      consented_msg_1L_chr <- paste0("Any files in ", path_to_R_dir_1L_chr, " with a matching pattern ",make_list_phrase(paste0("#' @importFrom ", pkgs_chr))," have been edited.")
    }
    if(where_1L_chr == "DESCRIPTION"){
      prompt_1L_chr <- paste0("Do you confirm that you want to edit the DESCRIPTION file in '",
                              path_to_pkg_root_1L_chr,"' to remove the packages ", make_list_phrase(pkgs_chr),
                              " from the 'Imports' list and add them to the 'Suggests' list", " ?")
      consented_msg_1L_chr <- paste0("The DESCRIPTION file in ", path_to_pkg_root_1L_chr, " has been updated to list the ", make_list_phrase(pkgs_chr), " packages as suggested rather than imported.")
    ...
    if(where_1L_chr %in% c("R", "DESCRIPTION")){
      write_with_consent(consented_fn = consented_fn,
                         ...
                         consent_1L_chr = consent_1L_chr,
                         consent_indcs_int = consent_indcs_int,
                         consented_args_ls = list(path_to_pkg_root_1L_chr = path_to_pkg_root_1L_chr, pkgs_chr = pkgs_chr, where_1L_chr = where_1L_chr),
...
write_dv_fl_to_loc <- function(ds_url_1L_chr = character(0),
                               dest_path_1L_chr,
                               repo_fl_fmt_1L_chr,
                               consent_1L_chr = "",
                               consent_indcs_int = 1L,
  ...
  if (lifecycle::is_present(ds_ui_1L_chr)) {
    lifecycle::deprecate_warn("0.1.10.9005",
                              "ready4::write_dv_fl_to_loc(ds_ui_1L_chr)",
  ...
  if(!is.null(ds_ls)){
    if(ds_ls$versionState != "DRAFT"){
      ...
      write_with_consent(consented_fn = write_ingested_dv_fl,
                         ...
                         consent_1L_chr = consent_1L_chr,
                         consent_indcs_int = consent_indcs_int,
                         consented_args_ls = list(ds_url_1L_chr = ds_url_1L_chr,
...
write_env_objs_to_dv <- function(env_objects_ls,
  ...
  if (identical(piggyback_to_1L_chr, character(0))) {
    ds_ls <- tryCatch(dataverse::get_dataset(ds_url_1L_chr, key = key_1L_chr), error = function(cond){NULL})
    if(is.null(ds_ls)){
...
write_examples <- function(path_1L_chr,
                           consent_1L_chr = "",
                           consent_indcs_int = 1L,
                           options_chr = c("Y", "N"),
                           type_1L_chr = "fn"){
...
write_extra_pkgs_to_actions <- function(path_to_dir_1L_chr,
                                        consent_1L_chr = "",
                                        consent_indcs_int = 1L,
                                        options_chr = c("Y", "N")){
...
write_fls_to_repo <- function(paths_chr,
  ...
  if(!identical(piggyback_to_1L_chr,character(0))){
    consented_fn <- function(paths_chr,
                             ...
                             prerelease_1L_lgl){
      ids_int <- NULL
      releases_xx <- get_gracefully(piggyback_to_1L_chr, fn = piggyback::pb_list,
                                    args_ls = list(tag = piggyback_tag_1L_chr, .token = ""))
...
write_library_metadata <- function(additions_tb = make_additions_tb(),
  ...
  if(!is.null(libraries_tb)){
    ...
    # if(!is.null(libraries_ls)){
    #   Not sure if a conditional check is needed here - would need to be implemented carefully
    # }
    env_objects_ls <- list()
    ...
    if(update_list_1L_lgl | update_table_1L_lgl){
      ...
      # if(!is.null(env_objects_ls$methods_tb){
      #   Not sure if a conditional check is needed here - would need to be implemented carefully
      # }
...
write_ingested_dv_fl <- function(ds_url_1L_chr = character(0),
                                 dest_path_1L_chr,
                                 repo_fl_fmt_1L_chr,
                                 consent_1L_chr = "",
                                 consent_indcs_int = 1L,
  ...
  if (lifecycle::is_present(ds_ui_1L_chr)) {
    lifecycle::deprecate_warn("0.1.10.9005",
                              "ready4::write_ingested_dv_fl(ds_ui_1L_chr)",
  ...
  object_xx <- get_gracefully(ds_url_1L_chr, fn = dataverse::get_file,
                              args_ls = list(file = ifelse(is.na(fl_id_1L_int), paste0(fl_nm_1L_chr, repo_fl_fmt_1L_chr), fl_id_1L_int),
