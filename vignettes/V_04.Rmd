...
There are currently four types of `ready4` framework code library:
...
A summary table of currently publicly available data stored in `ready4`'s Dataverses can be generated using the `get_dvs` function. A HTML version of the table can be displayed using `print_dvs`.
```{r}
y <- get_dvs()
```
```{r}
...
Currently, the main source of examples of `ready4` framework literate programs are the vignette articles from code libraries. A HTML table summarising these literate programs can be generated by applying the `print_vignettes` function to the output (`x`) from our prior call to the `make_pkg_extensions_tb` function. The table includes links to both the HTML version (for reading - and which displays program outputs) and RMD version (input only and which can be executed by R) of each literate program.
