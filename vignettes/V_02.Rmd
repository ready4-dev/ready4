...
### Core generics
`ready4` includes a number of core generic functions which describe the main types of method to be implemented by the `ready4` framework. Notably, the `ready4` package does not include methods for any of these core generics. Instead, methods are defined for these generics in [R packages that extend the `ready4` framework](V_04.html). A HTML table of the core generics bundled with `ready4` and examples of methods that implement each generic can be displayed using the `print_methods` function, using the `return_1L_chr = "core"` argument. 
