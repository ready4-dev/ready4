...
## Implementation
`ready4` provides a simple syntax that is used to apply all model modules developed with the [`ready4` framework](https://www.ready4-dev.com/).

## Use
A table that summarises the latest `ready4` syntax can be ingested from a periodically updated database using `get_methods_tb`. In the below example we will search for examples of where that syntax has been used to call modules from the [readyforwhatsnext model](https://readyforwhatsnext.org/). The value supplied to the `gh_repo_1L_chr` argument specifies the repository in which a dataset of readyforwhatsnext module libraries is stored.
